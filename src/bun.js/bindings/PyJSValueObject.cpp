...
struct PyFutureCallback {
    PyObject_HEAD
    PyObject* future; // The asyncio.Future to resolve/reject
...
{
    PyJSDictObject* wrapper = PyObject_New(PyJSDictObject, &PyJSDict_Type);
    if (!wrapper) {
        return nullptr;
    }

    // Initialize dict base - we don't use internal storage, our methods access JS directly
...
{
    PyJSListObject* wrapper = PyObject_New(PyJSListObject, &PyJSList_Type);
    if (!wrapper) {
        return nullptr;
    }

    // Initialize list base - we don't use internal storage, our methods access JS directly
...
static void pyjsdict_dealloc(PyObject* self)
{
    ...
    // Don't call PyDict_Type.tp_dealloc - we don't use internal dict storage
    PyObject_Del(self);
}
...
static void pyjslist_dealloc(PyObject* self)
{
    ...
    // Don't call PyList_Type.tp_dealloc - we don't use internal list storage
    PyObject_Del(self);
}
